...
async def main_loop(cfg_dict: dict, i2c_instance: SoftI2C):
    ...
    #Create a Queue for MQTT outgoing messages
    mqtt_q = Queue(maxsize=20)
    ...
    postprocessor_coro_handle = uasyncio.create_task(postprocessor_coro(
        ...
        outgoing_mqtt_queue=mqtt_q,
        button=botton,
        my_id=cfg["mqtt_client_id"],
        debug_prints=cfg["main_enable_debug_print"]
    ...
    mqtt_coro_handle = uasyncio.create_task(mqtt_coro(
        mqtt_queue=mqtt_q,
        cfg=cfg
    ))
    coro_handles.append(mqtt_coro_handle)
